...
function validate(instance, schema, strictValidation) {
  ...
  Object.keys(flatSchema).forEach(function(name) {
    ...
    var err = validateInstanceItem(schemaItem, instanceItem);
    if(err !== null) errors.invalid_type.push(err);
...
function validateInstanceItem(schemaItem, instanceItem) {
  ...
  let rv = {
    ...
    validate: function(p1, p2, p3) {
      ...
      if(typeof k !== 'undefined' && typeof v !== 'undefined') {
        errors = {invalid_type:[], undeclared:[], missing:[]};
        try{
          let schemaItem = traverseSchema(this._schema, k);
          v = coerce(k, v, this._schema, this);
          let err = validateInstanceItem(schemaItem, v);
          if(err) errors.invalid_type.push(err);
        } catch(err) {
          errors.invalid_type.push(err);
        }
      }else{
        errors = validate(this._instance, this._schema, options.allowed);
      }
